...
import type { CustomVRFCoordinatorV2Mock, FareCrashMock, FareToken } from '../typechain-types'
...
const func: DeployFunction = async function ({
  ...
  if (chainId === '31337' || TESTNET_DEPLOYMENT || RUNNING_TESTS) {
    ...
    const deploymentInfo = await deploy('FareCrashMock', {
      contract: 'FareCrashMock',
      from: owner,
      log: true,
      // autoMine: true,
      args: [
    ...
    if (!RUNNING_TESTS || LOCAL_DEV) {
      const crash = (await hre.ethers.getContract('FareCrashMock')) as FareCrashMock
      const fareToken = (await hre.ethers.getContract('FareToken')) as FareToken
      await fareToken.setWhitelistAddress(crash.address, true)
      console.log(
        `Set crashAddress(${crash.address}) to whitelistAddressList on fareToken(${fareToken.address})!`
      )
      await fareToken.setAllowContractMintBurn(crash.address, true)
      console.log('Added allow mint/burn for FareCrash to owner address.')

      await fareToken.connect(protocolSigner).setAllowContractMintBurn(crash.address, true)
      console.log('Added allow mint/burn for FareCrash to protocol address.')

      await fareToken.connect(hostSigner).setAllowContractMintBurn(crash.address, true)
      console.log('Added allow mint/burn for FareCrash to host address.')

      await fareToken.connect(userSigner).setAllowContractMintBurn(crash.address, true)
      console.log('Added allow mint/burn for FareCrash to user address.')
    }
