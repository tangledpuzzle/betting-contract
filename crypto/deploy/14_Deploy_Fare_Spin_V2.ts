...
import type { CustomVRFCoordinatorV2Mock, FareSpinV2Mock, FareToken } from '../typechain-types'
...
const func: DeployFunction = async function ({
  ...
  const { owner, resolver, protocol, host } = await getNamedAccounts()

  const { protocol: protocolSigner, host: hostSigner, user: userSigner } = await getNamedSigners()
  ...
  if (chainId === '31337' || TESTNET_DEPLOYMENT || RUNNING_TESTS) {
    ...
    const deploymentInfo = await deploy('FareSpinV2Mock', {
      contract: 'FareSpinV2Mock',
      from: owner,
      log: true,
      // autoMine: true,
      args: [
    ...
    if (!RUNNING_TESTS || LOCAL_DEV) {
      const spin = (await hre.ethers.getContract('FareSpinV2Mock')) as FareSpinV2Mock
      const fareToken = (await hre.ethers.getContract('FareToken')) as FareToken
      await fareToken.setWhitelistAddress(spin.address, true)
      console.log(
      ...
      console.log('Added allow mint/burn for FareSpinV2 to owner address.')

      await fareToken.connect(protocolSigner).setAllowContractMintBurn(spin.address, true)
      console.log('Added allow mint/burn for FareSpinV2 to protocol address.')

      await fareToken.connect(hostSigner).setAllowContractMintBurn(spin.address, true)
      console.log('Added allow mint/burn for FareSpinV2 to host address.')

      await fareToken.connect(userSigner).setAllowContractMintBurn(spin.address, true)
      console.log('Added allow mint/burn for FareSpinV2 to user address.')
    }
