...
const func: DeployFunction = async function ({
  ...
  if (chainId === '31337' || TESTNET_DEPLOYMENT || RUNNING_TESTS) {
    ...
    const deploymentInfo = await deploy('FareCoinFlipBaseSUContract', {
      contract: 'FareCoinFlipBaseSUContract',
      from: owner,
      log: true,
      // autoMine: true,
      args: [
    ...
    if (!RUNNING_TESTS || LOCAL_DEV) {
      ...
      console.log('Added allow mint/burn for FareCoinFlipBaseSUContract to owner address.')

      await fareToken.connect(protocolSigner).setAllowContractMintBurn(coinFlip.address, true)
      console.log('Added allow mint/burn for FareCoinFlipBaseSUContract to protocol address.')

      await fareToken.connect(hostSigner).setAllowContractMintBurn(coinFlip.address, true)
      console.log('Added allow mint/burn for FareCoinFlipBaseSUContract to host address.')

      await fareToken.connect(userSigner).setAllowContractMintBurn(coinFlip.address, true)
      console.log('Added allow mint/burn for FareCoinFlipBaseSUContract to user address.')
    ...
    if (!TESTNET_DEPLOYMENT || RUNNING_TESTS) {
      log('FareCoinFlipBaseSUContract')
      log('----------------------------------------------------')
