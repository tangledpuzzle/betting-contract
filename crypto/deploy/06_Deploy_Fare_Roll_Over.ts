...
import type { CustomVRFCoordinatorV2Mock, FareRollOverMock, FareToken } from '../typechain-types'
...
const func: DeployFunction = async function ({
  ...
  if (chainId === '31337' || TESTNET_DEPLOYMENT || RUNNING_TESTS) {
    ...
    const deploymentInfo = await deploy('FareRollOverMock', {
      contract: 'FareRollOverMock',
      from: owner,
      log: true,
      // autoMine: true,
      args: [
    ...
    if (!RUNNING_TESTS || LOCAL_DEV) {
      const rollOver = (await hre.ethers.getContract('FareRollOverMock')) as FareRollOverMock

      const fareToken = (await hre.ethers.getContract('FareToken')) as FareToken
      await fareToken.setWhitelistAddress(rollOver.address, true)
      console.log(
        `Set rollOverAddress(${rollOver.address}) to whitelistAddressList on fareToken(${fareToken.address})!`
      )
      await fareToken.setAllowContractMintBurn(rollOver.address, true)
      console.log('Added allow mint/burn for FareRollOver to owner address.')

      await fareToken.connect(protocolSigner).setAllowContractMintBurn(rollOver.address, true)
      console.log('Added allow mint/burn for FareRollOver to protocol address.')

      await fareToken.connect(hostSigner).setAllowContractMintBurn(rollOver.address, true)
      console.log('Added allow mint/burn for FareRollOver to host address.')

      await fareToken.connect(userSigner).setAllowContractMintBurn(rollOver.address, true)
      console.log('Added allow mint/burn for FareRollOver to user address.')
    }
