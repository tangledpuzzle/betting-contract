...
const main = async () => {
  ...
  for (let i = 0; i < repeatCount; i++) {
    ...
    const randomNumbers = await resolveKeccakMock.mockResolveWithNFT(randomNumberAmount)
    randomNumbers
  ...
  const filePath = `./pseudoRNSimulationResults/resolveKeccakForNFT/${Math.floor(
    Date.now() / 1000
  )}.json`
