...
describe('Deployment', () => {
  ...
  let coinFlip: FareCoinFlipNFTBasedPPV
  let link: LinkToken
  ...
  beforeEach(async () => {
    ...
    await deployments.fixture(['mocks', 'fare', 'ppv_nft', 'cf_nftBasedPPV'])
    ...
    coinFlip = (await ethers.getContract('FareCoinFlipNFTBasedPPV')) as FareCoinFlipNFTBasedPPV
  })

  it('Successful FareCoinFlipNFTBasedPPV Deployment', async () => {
    const FareCoinFlipNFTBasedPPVFactory = await ethers.getContractFactory(
      'FareCoinFlipNFTBasedPPV'
    )
    const FareCoinFlipNFTBasedPPVDeployed = await FareCoinFlipNFTBasedPPVFactory.deploy(
      {
        baseContractParams: {
    ...
    expect(await FareCoinFlipNFTBasedPPVDeployed.owner()).to.be.equal(owner)
  })
  ...
  it('Invalid fareTokenAddress should fail deployment', async () => {
    ...
    await expect(
      FareCoinFlipNFTBasedPPVFactory.deploy(
        {
          baseContractParams: {
...
describe('FareCoinFlipNFTBasedPPV', () => {
  const zeroAddress = ethers.constants.AddressZero
  let fare: FareToken
  let vrfCoordinator: CustomVRFCoordinatorV2Mock
  let airnodeRrpMock: AirnodeRrpMock
  ...
  describe('Constructor', () => {
    it('FareCoinFlipNFTBasedPPV has the correct FareToken address', async () => {
      const coinFlipFareToken = await coinFlip.fareToken()
      expect(coinFlipFareToken).to.equal(fare.address)
    })

    it('FareCoinFlipNFTBasedPPV and FareToken owner address is the same', async () => {
    ...
    it('FareCoinFlipNFTBasedPPV protocol address is correct', async () => {
      const actual = await coinFlip.protocolAddress()
      expect(actual).to.equal(protocol)
    })

    it('FareCoinFlipNFTBasedPPV protocol balance is 0 FARE', async () => {
      const actual = await fare.balanceOf(protocol)
      expect(actual).to.equal(BN.from(0))
    })

    it('FareCoinFlipNFTBasedPPV host address is correct', async () => {
      const actual = await coinFlip.hostAddress()
      expect(actual).to.equal(host)
    })

    it('FareCoinFlipNFTBasedPPV host balance is 0 FARE', async () => {
      const actual = await fare.balanceOf(host)
      expect(actual).to.equal(BN.from(0))
    })

    it('FareCoinFlipNFTBasedPPV precision is 1 ether', async () => {
      const actualPrecision = await coinFlip.PRECISION()
      expect(actualPrecision).to.eq(oneEther)
    })

    it('FareCoinFlipNFTBasedPPV ppv value is 0.01 ether which represents 1.00% (default)', async () => {
      const ppv = await coinFlip.protocolProbabilityValue()
      expect(ppv).to.equal(oneEther.div('100'))
    })

    it('FareCoinFlipNFTBasedPPV MIN_PROTOCOL_PROBABILITY_VALUE is 0.01 ether which represents 0.1% (default)', async () => {
    ...
    it('FareCoinFlipNFTBasedPPV HOST_REWARDS_PERCENTAGE value is 15% of the PPV which represents 0.15% (if ppv is 1%)', async () => {
    ...
    it('FareCoinFlipNFTBasedPPV PROTOCOL_REWARDS_PERCENTAGE value is 5% of the PPV which represents 0.05% (if ppv is 1%)', async () => {
    ...
    it('FareCoinFlipNFTBasedPPV MIN_BLOCK_AMOUNT_FOR_RANDOM_NUMBER_FAILURE is 200 (default)', async () => {
    ...
    it('FareCoinFlipNFTBasedPPV maxEntryCount is 20 (default)', async () => {
      const maxEntryCount = await coinFlip.maxEntryCount()
      expect(maxEntryCount).to.equal(defaultMaxEntryCount)
    })

    it('FareCoinFlipNFTBasedPPV has the correct FarePPVNFT address', async () => {
      const coinFlipFarePPVNFT = await coinFlip.farePPVNFT()
      expect(coinFlipFarePPVNFT).to.equal(ppvNFT.address)
    })

    it('FareCoinFlipNFTBasedPPV contractName is `FareCoinFlipNFTBasedPPV`', async () => {
      const contractName = await coinFlip.contractName()
      expect(contractName).to.equal('FareCoinFlipNFTBasedPPV')
    })
  })
  ...
  describe('NFT based protocol probability value', async () => {
    ...
    it('Should revert if tokenId does not exist', async () => {
      ...
      await expect(ppvNFT.tokenURI(100)).to.be.revertedWithCustomError(ppvNFT, 'NonExistingTokenId')
    })
