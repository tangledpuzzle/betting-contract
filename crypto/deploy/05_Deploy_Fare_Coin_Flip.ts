...
import type { CustomVRFCoordinatorV2Mock, FareCoinFlipMock, FareToken } from '../typechain-types'
...
const func: DeployFunction = async function ({
  ...
  if (chainId === '31337' || TESTNET_DEPLOYMENT || RUNNING_TESTS) {
    ...
    const deploymentInfo = await deploy('FareCoinFlipMock', {
      contract: 'FareCoinFlipMock',
      from: owner,
      log: true,
      // autoMine: true,
      args: [
    ...
    if (!RUNNING_TESTS || LOCAL_DEV) {
      const coinFlip = (await hre.ethers.getContract('FareCoinFlipMock')) as FareCoinFlipMock
      ...
      console.log('Added allow mint/burn for FareCoinFlip to owner address.')

      await fareToken.connect(protocolSigner).setAllowContractMintBurn(coinFlip.address, true)
      console.log('Added allow mint/burn for FareCoinFlip to protocol address.')

      await fareToken.connect(hostSigner).setAllowContractMintBurn(coinFlip.address, true)
      console.log('Added allow mint/burn for FareCoinFlip to host address.')

      await fareToken.connect(userSigner).setAllowContractMintBurn(coinFlip.address, true)
      console.log('Added allow mint/burn for FareCoinFlip to user address.')
