...
// @NOTE: Pseudo randomNumbers being distributed fairly after (mod 100) does not mean they are well distributed directly.
...
const main = async () => {
  ...
  for (let i = 0; i < repeatCount; i++) {
    ...
    // from 1 to 10
    const randomNumberAmountsArray = Array(Math.floor(Math.random() * 10) + 1)
      .fill(1)
      .map((item) => Math.floor(Math.random() * 10) + 1)

    const randomNumbers = await resolveKeccakMock.mockBatchResolve(randomNumberAmountsArray)
    randomNumbers
  ...
  const filePath = `./pseudoRNSimulationResults/batchResolveKeccak/${Math.floor(
    Date.now() / 1000
  )}.json`
